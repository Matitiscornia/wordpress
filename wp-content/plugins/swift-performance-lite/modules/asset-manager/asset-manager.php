...
class Swift_Performance_Asset_Manager {
	...
	public function asset_manager_callback($buffer){
			...
			if ($skip_cf7 && ((isset($node->href) && strpos($node->href, 'contact-form-7/includes') !== false) || ($node->tag == 'script' && strpos($node->innertext, 'wp-json\/contact-form-7\/v') !== false)) ){
				...
				if ($node->tag == 'link'){
					...
					if ($prebuild_booster->check($node->href)){
						...
						// Webp Background
						if (Swift_Performance::check_option('serve-webp', 'picture') && Swift_Performance::check_option('serve-webp-background', 1)){
							$_css = preg_replace_callback('~url\s?\((\'|")?((((?!\)).)*)\.(png|jpe?g))(\'|")?\)~', array('Swift_Performance_Asset_Manager', 'webp_background'), $_css);
						}
		...
		if (Swift_Performance::check_option('serve-webp', 'picture') && Swift_Performance::check_option('serve-webp-background', 1)){
			$_html = preg_replace_callback('~url\s?\((&quot;|\'|")?((((?!\)).)*)\.(png|jpe?g))(&quot;|\'|")?\)~', array('Swift_Performance_Asset_Manager', 'webp_background'), $_html);
		}
      ...
      public function lazyload_images($args, $id){
		if (!isset($args['src']) || preg_match('~^data~', $args['src'])){
			...
			// Force sizes
				...
				// Maybe WebP
				$maybe_webp = (file_exists(trailingslashit($upload_dir['basedir']) . $intermediate['path'] . '.webp') ? '.webp' : '');
	                  $lazy_load_src[0] = str_replace(basename($args['src']), $intermediate['file'], $args['src']) . $maybe_webp;
	            }
	            else {
	...
	public static function get_webp_filename($original_src){
		if (preg_match('~^/(\w+)~', $original_src)){
			$webp_path = ABSPATH . Swift_Performance::canonicalize($original_src) . '.webp';
		}
		else {
